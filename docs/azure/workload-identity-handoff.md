...
The helper rejects an unrelated identity subscription/tenant, malformed identity resource ID/client ID, a namespace mismatch, a missing selected cluster, or applied federation with a different issuer, subject or token-exchange audience. Federation coverage comes from the actual AKS output metadata, not only a desired ServiceAccount list.
